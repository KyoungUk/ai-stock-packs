...
- `resistance`가 빈 배열이면 **"이번 주 도달 가능 거리에 유의미한 매물대 없음"**이라는 뜻이다(M09a
  임계값 보정 이후). 저항이 없다고 좋게 해석하지 말 것 — 오히려 이번 주 후보에서 빼는 근거가 된다.
- `excluded` 배열에 있는 종목(예: 상장 이력 부족)은 논의 대상이 아니다.
- `schema_version`이 2 미만이면 임계값 보정 전 팩이므로 저항값을 신뢰하지 말 것.
